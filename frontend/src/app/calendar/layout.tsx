...
export const LoginRequired = <T extends any>(WrappedComponent: React.FC<T>) => {
  const InnerFunction: React.FC<T> = (props: T) => {
    const router = useRouter();
    const [loaded, setLoaded] = useState(false);
    useEffect(() => {
      // SSR
      //Node // window, localstorage not exists, cannot use useState(in nextjs)
      // CSR
      //Chrome, Firefox, Explorer  // window, localstorage exists
      client
        .GET("/user/me")
        .then(({ data }) => {
          if (data?.id) {
